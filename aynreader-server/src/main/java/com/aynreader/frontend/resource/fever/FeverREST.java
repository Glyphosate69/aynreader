package com.aynreader.frontend.resource.fever;

import com.aynreader.backend.dao.FeedCategoryDAO;
import com.aynreader.backend.dao.FeedEntryDAO;
...
import com.aynreader.frontend.resource.fever.FeverResponse.FeverFavicon;
import com.aynreader.frontend.resource.fever.FeverResponse.FeverFeed;
import com.aynreader.frontend.resource.fever.FeverResponse.FeverFeedGroup;
import com.aynreader.frontend.resource.fever.FeverResponse.FeverGroup;
import com.aynreader.frontend.resource.fever.FeverResponse.FeverItem;
...
public class FeverREST {
    ...
    private static final String PATH =
            "/user/{userId}{optionalTrailingFever : (/fever)?}{optionalTrailingSlash : (/)?}";
    ...
    public FeverResponse formUrlencoded(
            @Context UriInfo uri,
    ...
    // workaround for some readers that post data using MultiPart FormData instead of the classic
    // POST
    // e.g. Raven Reader
    ...
    @Transactional
    @Operation(hidden = true)
    public FeverResponse formData(
            @Context UriInfo uri, @PathParam("userId") Long userId, MultipartFormDataInput form) {
        Map<String, String> params = new HashMap<>();
        uri.getQueryParameters().forEach((k, v) -> params.put(k, v.getFirst()));
        form.getValues()
                .forEach(
    ...
    public FeverResponse handle(long userId, Map<String, String> params) {
        ...
        if (params.containsKey("items")) {
            if (params.containsKey("with_ids")) {
                ...
                Long sinceId =
                        Optional.ofNullable(params.get("since_id"))
                                .filter(StringUtils::isNotBlank)
                                .map(Long::valueOf)
                                .orElse(null);
                Long maxId =
                        Optional.ofNullable(params.get("max_id"))
                                .filter(StringUtils::isNotBlank)
                                .map(Long::valueOf)
                                .orElse(null);
        ...
        if (params.containsKey("mark") && params.containsKey("id") && params.containsKey("as")) {
            ...
            Instant insertedBefore =
                    before == null ? null : Instant.ofEpochSecond(Long.parseLong(before));
        ...
        return subscriptions.stream()
                .collect(
                        Collectors.groupingBy(
                                s -> s.getCategory() == null ? 0 : s.getCategory().getId()))
                .entrySet()
                .stream()
                .map(
                        e -> {
                            ...
                            fg.setFeedIds(
                                    e.getValue().stream().map(FeedSubscription::getId).toList());
                            return fg;
                        })
                .toList();
    }
        ...
        return subscriptions.stream()
                .map(
                        s -> {
                            ...
                            f.setSpark(false);
                            f.setLastUpdatedOnTime(
                                    s.getFeed().getLastUpdated() == null
                                            ? 0
                                            : s.getFeed().getLastUpdated().getEpochSecond());
                            return f;
                        })
                .toList();
    }
    ...
    private List<Long> buildSavedItemIds(User user) {
        List<FeedEntryStatus> statuses =
                feedEntryStatusDAO.findStarred(
                        user, null, null, 0, SAVED_ITEM_IDS_BATCH_SIZE, ReadingOrder.DESC, false);
        return statuses.stream().map(s -> s.getEntry().getId()).toList();
    }

    private List<FeverItem> buildItems(
            User user, List<FeedSubscription> subscriptions, List<String> entryIds) {
        List<FeverItem> items = new ArrayList<>();

        Map<Long, FeedSubscription> subscriptionsByFeedId =
                subscriptions.stream().collect(Collectors.toMap(s -> s.getFeed().getId(), s -> s));
    ...
    private List<FeverItem> buildItems(
            User user, List<FeedSubscription> subscriptions, Long sinceId, Long maxId) {
        List<FeedEntryStatus> statuses =
                feedEntryStatusDAO.findBySubscriptions(
                        user,
                        subscriptions,
                        ...
                        s -> {
                            ...
                            f.setData(
                                    String.format(
                                            "data:%s;base64,%s",
                                            favicon.mediaType(),
                                            Base64.getEncoder().encodeToString(favicon.icon())));
                            return f;
                        })
                .toList();
    }
    ...
    private void mark(User user, String source, long id, String action, Instant insertedBefore) {
        if ("item".equals(source)) {
            ...
            feedEntryService.markSubscriptionEntries(
                    user, Collections.singletonList(subscription), null, insertedBefore, null);
            ...
            feedEntryService.markSubscriptionEntries(
                    user, subscriptions, null, insertedBefore, null);
        }
    }
}
