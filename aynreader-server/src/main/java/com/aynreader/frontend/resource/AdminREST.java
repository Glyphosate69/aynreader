...
import com.aynreader.frontend.model.request.AdminSaveUserRequest;
...
public class AdminREST {
        ...
        if (id == null) {
            ...
            if (req.getId().equals(user.getId()) && !req.isEnabled()) {
                return Response.status(Status.FORBIDDEN)
                        .entity("You cannot disable your own account.")
                        .build();
            }
            ...
            if (StringUtils.isNotBlank(req.getPassword())) {
                u.setPassword(
                        encryptionService.getEncryptedPassword(req.getPassword(), u.getSalt()));
            }
            ...
            if (req.isAdmin() && !roles.contains(Role.ADMIN)) {
                ...
                if (userRoleDAO.countAdmins() == 1) {
                    return Response.status(Status.FORBIDDEN)
                            .entity("You cannot remove the admin role from the last admin user.")
                            .build();
                }
    ...
    public UserModel adminGetUser(
        ...
        userModel.setAdmin(
                userRoleDAO.findAll(u).stream().anyMatch(r -> r.getRole() == Role.ADMIN));
        return userModel;
    }
    ...
    public Response adminDeleteUser(@Parameter(required = true) IDRequest req) {
        ...
        if (user.getId().equals(u.getId())) {
            return Response.status(Status.FORBIDDEN)
                    .entity("You cannot delete your own user.")
                    .build();
        }
