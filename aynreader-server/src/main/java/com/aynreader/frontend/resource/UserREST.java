...
import com.aynreader.backend.model.UserSettings.IconDisplayMode;
import com.aynreader.backend.model.UserSettings.PushNotificationUserSettings;
import com.aynreader.backend.model.UserSettings.ReadingMode;
import com.aynreader.backend.model.UserSettings.ReadingOrder;
import com.aynreader.backend.model.UserSettings.ScrollMode;
import com.aynreader.backend.service.MailService;
import com.aynreader.backend.service.PasswordEncryptionService;
...
import com.aynreader.frontend.model.Settings;
import com.aynreader.frontend.model.Settings.PushNotificationSettings;
import com.aynreader.frontend.model.UserModel;
import com.aynreader.frontend.model.request.InitialSetupRequest;
import com.aynreader.frontend.model.request.PasswordResetConfirmationRequest;
import com.aynreader.frontend.model.request.PasswordResetRequest;
import com.aynreader.frontend.model.request.ProfileModificationRequest;
import com.aynreader.frontend.model.request.RegistrationRequest;
...
public class UserREST {
    ...
    public Response saveUserSettings(@Parameter(required = true) Settings settings) {
        ...
        s.setCustomCss(settings.getCustomCss());
        s.setCustomJs(
                AynReaderConstants.USERNAME_DEMO.equals(user.getName())
                        ? ""
    ...
    public Response sendTestPushNotification(
        ...
        sub.setTitle("Ayn Reader OS Test Feed");
    ...
    public Response initialSetup(@Valid @Parameter(required = true) InitialSetupRequest req) {
        ...
        if (config.users().createDemoAccount()) {
            User demo = userDAO.findByName(AynReaderConstants.USERNAME_DEMO);
            if (demo == null) {
    ...
    public Response sendPasswordReset(@Valid @Parameter(required = true) PasswordResetRequest req) {
        if (!config.passwordRecoveryEnabled()) {
            throw new IllegalArgumentException(
                    "Password recovery is not enabled on this Ayn Reader OS instance");
        }
