...
import com.aynreader.backend.favicon.Favicon;
import com.aynreader.backend.feed.FeedEntryKeyword;
import com.aynreader.backend.feed.FeedFetcher;
...
import com.aynreader.backend.model.UserSettings.ReadingMode;
import com.aynreader.backend.model.UserSettings.ReadingOrder;
import com.aynreader.backend.opml.OPMLExporter;
import com.aynreader.backend.opml.OPMLImporter;
import com.aynreader.backend.service.FeedEntryFilteringService;
import com.aynreader.backend.service.FeedEntryFilteringService.FeedEntryFilterException;
import com.aynreader.backend.service.FeedEntryService;
import com.aynreader.backend.service.FeedFaviconService;
import com.aynreader.backend.service.FeedSubscriptionService;
import com.aynreader.backend.service.FeedSubscriptionService.ForceFeedRefreshTooSoonException;
import com.aynreader.frontend.model.Entries;
import com.aynreader.frontend.model.Entry;
import com.aynreader.frontend.model.FeedInfo;
import com.aynreader.frontend.model.Subscription;
import com.aynreader.frontend.model.UnreadCount;
import com.aynreader.frontend.model.request.FeedInfoRequest;
import com.aynreader.frontend.model.request.FeedModificationRequest;
import com.aynreader.frontend.model.request.IDRequest;
import com.aynreader.frontend.model.request.MarkRequest;
import com.aynreader.frontend.model.request.ScrapeSubscribeRequest;
import com.aynreader.frontend.model.request.SubscribeRequest;
...
public class FeedREST {
    ...
    private static FeedEntry initTestEntry() {
        ...
        entry.setUrl("https://aynreader.local/");
    ...
    public Response getFeedEntries(
            ...
            @Parameter(description = "all entries or only unread ones", required = true)
                    @DefaultValue("unread")
                    @QueryParam("readType")
            ...
            @Parameter(description = "limit for paging, default 20, maximum 1000")
                    ...
                    @QueryParam("limit")
                    int limit,
            @Parameter(description = "ordering") @QueryParam("order") @DefaultValue("desc")
                    ReadingOrder order,
            @Parameter(
                            description =
        ...
        List<FeedEntryKeyword> entryKeywords =
                FeedEntryKeyword.fromQueryString(StringUtils.trimToNull(keywords));
        ...
        if (subscription != null) {
            ...
            return Response.status(Status.NOT_FOUND)
                    .entity("<message>feed not found</message>")
                    .build();
        }
    ...
    public Response getFeedEntriesAsFeed(
            ...
            @Parameter(description = "limit for paging, default 20, maximum 1000")
                    ...
                    @QueryParam("limit")
                    int limit,
            @Parameter(description = "date ordering") @QueryParam("order") @DefaultValue("desc")
                    ReadingOrder order,
            @Parameter(
                            description =
                                    "search for keywords in either the title or the content of the entries, separated by spaces")
        ...
        feed.setTitle("Ayn Reader OS - " + entries.getName());
        feed.setDescription("Ayn Reader OS - " + entries.getName());
    ...
    private FeedInfo fetchFeedInternal(String url) {
        ...
        url = prependHttp(url);
        try {
            FeedFetcherResult feedFetcherResult =
                    feedFetcher.fetch(url, true, null, null, null, null);
            info = new FeedInfo();
    ...
    @APIResponse(
            ...
            content = {
                @Content(
                        mediaType = "application/json",
                        schema = @Schema(implementation = FeedInfo.class))
            })
    @APIResponse(responseCode = "404", description = "feed not found")
    ...
    public Response markFeedEntries(
        ...
        Instant olderThan =
                req.getOlderThan() == null ? null : Instant.ofEpochMilli(req.getOlderThan());
        Instant insertedBefore =
                req.getInsertedBefore() == null
                        ? null
                        : Instant.ofEpochMilli(req.getInsertedBefore());
        String keywords = req.getKeywords();
        List<FeedEntryKeyword> entryKeywords = FeedEntryKeyword.fromQueryString(keywords);

        User user = authenticationContext.getCurrentUser();
        FeedSubscription subscription =
                feedSubscriptionDAO.findById(user, Long.valueOf(req.getId()));
        if (subscription != null) {
    ...
    public Response getFeedFavicon(
        ...
        if (feed.getLastUpdated() == null) {
            // the feed has never been fetched yet so the iconUrl field hasn't been updated yet, and
            // the
            // icon can't be fetched
            // this can happen for newly subscribed feeds:
            // the feed has been added in the tree in the web client and the favicon is being
            // fetched,
    ...
    public Response subscribe(
            @Valid @Parameter(description = "subscription request", required = true)
        ...
        try {
            ...
            long subscriptionId =
                    feedSubscriptionService.subscribe(
                            user, info.getUrl(), req.getTitle(), category, 0);
    ...
    @POST
    @Path("/scrape/subscribe")
    @Transactional
    @Operation(
            summary = "Subscribe to a scraped page",
            description = "Create a feed from a scraped page")
    @APIResponse(
            responseCode = "200",
            content = {
                @Content(
    ...
    public Response subscribeScraped(
            @Valid @Parameter(description = "scraped subscription request", required = true)
                    ScrapeSubscribeRequest req) {
        Preconditions.checkNotNull(req);
        Preconditions.checkNotNull(req.getTitle());
        Preconditions.checkNotNull(req.getUrl());
        Preconditions.checkNotNull(req.getItemSelector());
        ...
        try {
            ...
            User user = authenticationContext.getCurrentUser();
            long subscriptionId =
                    feedSubscriptionService.subscribeScraped(
                            user,
                            prependHttp(req.getUrl()),
                            req.getItemSelector(),
                            req.getTitleSelector(),
                            req.getDescriptionSelector(),
                            req.getUrlSelector(),
                            req.getImageSelector(),
                            req.getDateSelector(),
                            req.getTitle(),
                            category,
                            0);
            ...
            log.error(
                    "Failed to create scraped feed for URL {}: {}",
                    req.getUrl(),
                    e.getMessage(),
                    e);
            return Response.status(Status.SERVICE_UNAVAILABLE)
    ...
    @Transactional
    @Operation(summary = "Subscribe to a feed", description = "Subscribe to a feed")
    public Response subscribeFromUrl(
    ...
    public Response modifyFeed(
            @Valid @Parameter(description = "subscription id", required = true)
                    FeedModificationRequest req) {
        Preconditions.checkNotNull(req);
        Preconditions.checkNotNull(req.getId());
    ...
    public Response importOpml(
            @Parameter(description = "ompl file", required = true) @RestForm("file") String opml) {
        User user = authenticationContext.getCurrentUser();
        if (AynReaderConstants.USERNAME_DEMO.equals(user.getName())) {
            return Response.status(Status.FORBIDDEN)
                    .entity("Import is disabled for the demo account")
                    .build();
        }
        try {
