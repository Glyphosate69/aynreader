...
import com.aynreader.backend.model.FeedEntryTag;
...
public class Entry implements Serializable {
    ...
    public static Entry build(FeedEntryStatus status, boolean proxyImages) {
        ...
        entry.setFeedName(sourceName(sub, feedEntry.getUrl()));
        entry.setFeedId(String.valueOf(sub.getId()));
        entry.setFeedUrl(sub.getFeed().getUrl());
        entry.setFeedLink(sourceLink(sub, feedEntry.getUrl()));
        ...
        if (content != null) {
            ...
            entry.setContent(
                    proxyImages
                            ? FeedUtils.proxyImages(content.getContent())
                            : content.getContent());
            entry.setAuthor(content.getAuthor());

            entry.setEnclosureType(content.getEnclosureType());
            entry.setEnclosureUrl(
                    proxyImages && Strings.CS.contains(content.getEnclosureType(), "image")
            ...
            entry.setMediaThumbnailUrl(
                    proxyImages
                            ? FeedUtils.proxyImage(content.getMediaThumbnailUrl())
                            : content.getMediaThumbnailUrl());
    ...
    private static String sourceName(FeedSubscription subscription, String entryUrl) {
        String host = isGoogleNewsFeed(subscription.getFeed().getUrl()) ? host(entryUrl) : null;
        return host == null ? subscription.getTitle() : host;
    }

    private static String sourceLink(FeedSubscription subscription, String entryUrl) {
        return isGoogleNewsFeed(subscription.getFeed().getUrl()) && host(entryUrl) != null
                ? entryUrl
                : subscription.getFeed().getLink();
    }

    private static boolean isGoogleNewsFeed(String url) {
        if (url == null) {
            return false;
        }
        try {
    ...
    private static String host(String url) {
        ...
        try {
            ...
            if (host == null || "news.google.com".equalsIgnoreCase(host)) {
                return null;
            }
