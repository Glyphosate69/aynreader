...
public class FeedEntryService {
    ...
    public boolean updateUrlIfChanged(Feed feed, Entry entry) {
        FeedEntry existing = find(feed, entry);
        if (existing == null || entry.url() == null || entry.url().equals(existing.getUrl())) {
            return false;
        }

        existing.setUrl(FeedUtils.truncate(entry.url(), 2048));
        return true;
    }
