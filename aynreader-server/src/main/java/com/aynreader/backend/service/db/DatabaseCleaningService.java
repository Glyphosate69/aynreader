...
import com.aynreader.backend.dao.FeedEntryContentDAO;
import com.aynreader.backend.dao.FeedEntryDAO;
import com.aynreader.backend.dao.FeedEntryDAO.FeedCapacity;
import com.aynreader.backend.dao.FeedEntryStatusDAO;
import com.aynreader.backend.dao.UnitOfWork;
import com.aynreader.backend.model.AbstractModel;
...
public class DatabaseCleaningService {
        ...
        do {
            ...
            for (Feed feed : feeds) {
                ...
                do {
                    ...
                    entriesDeleted =
                            unitOfWork.call(() -> feedEntryDAO.delete(feed.getId(), batchSize));
    ...
    public void cleanEntriesForFeedsExceedingCapacity(final int maxFeedCapacity) {
        ...
        while (true) {
            ...
            List<FeedCapacity> feeds =
                    unitOfWork.call(
                            () ->
                                    feedEntryDAO.findFeedsExceedingCapacity(
                                            maxFeedCapacity, batchSize, keepStarredEntries));
            if (feeds.isEmpty()) {
                break;
            }
            ...
            for (final FeedCapacity feed : feeds) {
                ...
                do {
                    if (Thread.currentThread().isInterrupted()) {
                        log.info(
                                "interrupted, stopping cleanup of entries exceeding feed capacity");
                        return;
                    }
                    ...
                    deleted =
                            unitOfWork.call(
                                    () ->
                                            feedEntryDAO.deleteOldEntries(
                                                    feed.id(),
                                                    Math.min(batchSize, rem),
                                                    keepStarredEntries));
    ...
    public void cleanEntriesOlderThan(final Instant olderThan) {
        ...
        do {
            ...
            deleted =
                    unitOfWork.call(
                            () ->
                                    feedEntryDAO.deleteEntriesOlderThan(
                                            olderThan, batchSize, keepStarredEntries));
            entriesDeletedMeter.mark(deleted);
            total += deleted;
    ...
    public void cleanStatusesOlderThan(final Instant olderThan) {
        ...
        do {
            ...
            deleted =
                    unitOfWork.call(
                            () -> feedEntryStatusDAO.deleteOldStatuses(olderThan, batchSize));
            total += deleted;
