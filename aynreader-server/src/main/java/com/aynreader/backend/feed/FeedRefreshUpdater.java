...
public class FeedRefreshUpdater {
    ...
    private final GoogleNewsUrlDecoder googleNewsUrlDecoder;
    ...
    public FeedRefreshUpdater(
            ...
            GoogleNewsUrlDecoder googleNewsUrlDecoder) {
        this.unitOfWork = unitOfWork;
        this.feedService = feedService;
        this.feedEntryService = feedEntryService;
        this.feedSubscriptionDAO = feedSubscriptionDAO;
        this.googleNewsUrlDecoder = googleNewsUrlDecoder;
    ...
    private AddEntryResult addEntry(
            final Feed feed, final Entry entry, final List<FeedSubscription> subscriptions) {
    ...
    public FeedRefreshUpdaterResult update(Feed feed, List<Entry> entries) {
        ...
        long inserted = 0;
        Map<FeedSubscription, List<FeedEntry>> insertedUnreadEntriesBySubscription =
                new HashMap<>();

        if (!entries.isEmpty()) {
            List<FeedSubscription> subscriptions = null;
            List<Entry> decodedEntries = new ArrayList<>(entries.size());
            for (Entry entry : entries) {
                decodedEntries.add(googleNewsUrlDecoder.decode(entry));
            }
            unitOfWork.run(
                    () ->
                            decodedEntries.forEach(
                                    entry -> feedEntryService.updateUrlIfChanged(feed, entry)));
            List<Entry> newEntries =
                    unitOfWork.call(
                            () -> feedEntryService.removeExistingEntries(feed, decodedEntries));
    ...
    private record AddEntryResult(
            ...
            Set<FeedSubscription> subscriptionsForWhichEntryIsUnread) {}
