...
const useStyles = tss
    ...
    .create(({ colorScheme, read, expanded, viewMode, rtl, showSelectionIndicator, maxWidth, fontSizePercentage }) => {
        let backgroundColor: string
        if (colorScheme === "dark") {
            backgroundColor = read ? "var(--ayn-panel)" : "var(--ayn-dark-surface-high)"
        } else {
            backgroundColor = read && !expanded ? "var(--ayn-gray-50)" : "var(--ayn-panel)"
        }
        ...
        if (!expanded && !read) {
            backgroundHoverColor = colorScheme === "dark" ? "var(--ayn-dark-acid-muted)" : "var(--ayn-acid-50)"
        }

        let paperBorderLeftColor = ""
        if (showSelectionIndicator) {
            paperBorderLeftColor = "var(--ayn-acid) !important"
        }
