...
- On shutdown, give the feed refresh engine some time to finish refreshing feeds before killing it. This is controlled by the new `aynreader.shutdown-timeout` setting, which defaults to 2 seconds
...
- The default value of `aynreader.http-client.block-local-addresses` is now false, allowing users to subscribe to feeds only available on their local network. This may be a security risk (SSRF) if your instance is accessible by untrusted users, so you may want to set it to true if you host a public instance of AynReader with user registeration enabled.
- When `aynreader.http-client.block-local-addresses` is enabled, SSRF is now also mitigated by blocking public websites redirecting to local ones.

## [6.2.0]

- Starred entries are no longer deleted after a certain amount of time, they are now kept indefinitely. The new `aynreader.database.cleanup.keep-starred-entries` setting can be disabled to restore the previous behavior if you want to keep deleting starred entries during normal entries cleanup (#1581)
...
- Use browser preferred language instead of English when using AynReader for the first time (#2018)
...
- When booting AynReader for the first time, the default "admin" account is no longer created automatically. A setup wizard will guide you through the creation of an admin account
- Default password complexity requirements have been lowered for local network deployments, where strict password rules are often unnecessary. The `aynreader.users.strict-password-policy` setting has been replaced by `aynreader.users.minimum-password-length` with a default value of `4` (#1916)
- Email addresses are no longer required when creating users and when they update their profile. The `aynreader.users.email-address-required` setting has been added to restore the previous behavior (#1914)
- Java 25+ is now required to build and run AynReader
...
- The process in the docker native image is now called "aynreader" instead of "application"
...
- Added a link in the README to the [documentation](https://athou.github.io/aynreader/documentation/custom-css/) of the new CSS classes
...
- AynReader wil try to match the language of the browser before defaulting to english (#1767)
...
- To better respect the bandwidth of feed owners, the default value of `aynreader.feed-refresh.interval-empirical` is now true. This means feeds no longer refresh exactly every 5 minutes (the default value of `aynreader.feed-refresh.interval`) but between 5 minutes and 4 hours (the default value of the new `aynreader.feed-refresh.max-interval` setting). The interval is calculated based on feed activity, so highly active feeds refresh more often (#1677)
- Many previously hardcoded values used in feed refresh interval calculation are now exposed as settings (#1677)
- Access to local addresses is now blocked to mitigate server-side request forgery (SSRF) attacks, which could potentially expose internal resources. You might want to disable the new `aynreader.http-client.block-local-addresses` setting if you subscribe to feeds only available on your local network and you trust all your users
...
- AynReader now honors the Retry-After response header and will not try to refresh a feed sooner than the value of this header
...
- AynReader now honors the Cache-Control response header and will not try to refresh a feed sooner than its max-age property (#1615)
- Added support for compilation with JDK 23+. If you're building AynReader from sources with a JDK 17 or 21, you may need to update it to the most recent patch version to support `-proc:full` (#1618)
...
AynReader is now powered by Quarkus instead of Dropwizard. Read the rationale behind this change in
the [announcement](https://github.com/Athou/aynreader/discussions/1517).
The gist of it is that AynReader can now be compiled to a native binary, resulting in blazing fast startup times (around
0.3s) and very low memory footprint (< 50M).

- AynReader now has a different package for each supported database.
    - If you are deploying AynReader with a precompiled package, please
      read [this section of the README](https://github.com/Athou/aynreader/tree/master?tab=readme-ov-file#download-a-precompiled-package).
    - If you are building AynReader from sources, please
      read [this section of the README](https://github.com/Athou/aynreader/tree/master?tab=readme-ov-file#build-from-sources).
    - If you are using the Docker image, please read the instructions on
      the [Docker Hub page](https://hub.docker.com/r/athou/aynreader).
- Due to the switch to Quarkus, the way AynReader is configured is very different (the `config.yml` file is gone).
  Please
  read [this section of the README](https://github.com/Athou/aynreader/tree/master?tab=readme-ov-file#configuration).
...
- The H2 migration tool that automatically upgrades H2 databases from format 2 to 3 has been removed. If you're using
  the H2 embedded database, please upgrade to at least version 4.3.0 before upgrading to AynReader 5.0.0.
...
- h2 (the embedded database) has been upgraded to 2.2.224
    - this version uses a different file format than 2.1.x, the first time you start AynReader with this version, the
...
- added a memory management section to the readme, reading it is recommended if you are running AynReader on a server
  with limited memory
...
- the admin connector on port 8084 is now disabled in config.yml.example. Disabling it in your config.yml is
  recommended (see https://github.com/Athou/aynreader/commit/929df60f09cce56020b0962ab111cd8349b271b0)
...
- add a setting to hide aynreader from search engines (exposes a robots.txt file, enabled by default)
...
- add a button to open AynReader in a new tab and a button to open options when using the browser extension
...
- add compatibility with the new version of the AynReader browser extension
...
- add a standalone donate page with all ways to support AynReader
- fix an issue introduced in 3.1.0 that could make AynReader not refresh feeds as fast as before on instances with lots
  of feeds
...
- only apply hover effect for unread entries (same as aynreader v2)
...
- restore hover effect from aynreader 2.x
...
- Docker images are now automatically built and available at https://hub.docker.com/r/athou/aynreader
...
- fixed a bug that made aynreader store entry contents multiple times
- fixed a bug that prevented the app to be used as an installed app on mobile devices if the context path of aynreader
  was not "/"
...
- improve performance for instances with the heavy load setting enabled by preventing AynReader from fetching feeds from
...
- The backend has been completely rewritten using Dropwizard instead of TomEE, resulting in a lot less memory
  ...
  See the README on how to build AynReader from now on.
- AynReader should no longer fetch the same feed multiple times in a row
